// EZ:Created from quiat9.js
define(['timeAPI','pipScorer','underscore'], function(APIConstructor, Scorer, _) {
	...
	**/
	...
	function iatExtension(options)
	{
		...
		var iatObj =
		{
            ...
            alertIfDataMaxedOut:true, // if data is too long for qualtrics warn user
            shortData: false, // save a short version of data, good for trials with long stimuli (qualtrics can only save 20k characters).
		...
		*/
		...
		API.addStimulusSets({
			...
			att1 :
			[{
                data: {alias:piCurrent.shortData ? att1.$id : att1.name},
				inherit : 'target',
			...
			att2 :
			[{
                data: {alias:piCurrent.shortData ? att2.$id : att2.name},
				inherit : 'target',
			...
			cat1 :
			[{
                data: {alias:piCurrent.shortData ? cat1.$id : cat1.name},
				inherit : 'target',
			...
			cat2 :
			[{
                data: {alias:piCurrent.shortData ? cat2.$id : cat2.name},
				inherit : 'target',
		 ...
		 *	Media Sets
		 */
        function getMediaSet(src){ 
            return piCurrent.shortData
                ? src.stimulusMedia.map(function(media,i){ return _.set(media, 'alias', src.$id + 's' + (i+1)); }) // eg a1s1
                : src.stimulusMedia;
        }

		API.addMediaSets({
			att1 : getMediaSet(att1), att2 :getMediaSet(att2),
			cat1 : getMediaSet(cat1), cat2 :getMediaSet(cat2)
		});
		...
		var blockCondition = piCurrent.shortData 
            ? blockParamsCats.left1.$id + ',' + blockParamsCats.right1.$id
		...
		blockCondition = piCurrent.shortData
            ? blockParamsCombined.left2.$id + '/' + blockParamsCombined.left1.$id + ',' + blockParamsCombined.right2.$id + '/' + blockParamsCombined.right1.$id
            : blockParamsCombined.left2.name + '/' + blockParamsCombined.left1.name + ',' + blockParamsCombined.right2.name + '/' + blockParamsCombined.right1.name;
