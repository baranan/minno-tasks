	...
	**/
	...
	function epExtension(options)
	{
		...
		var epObj =
		{
			...
			instructions : {
				//Block 1
				firstBlock : '<div><p style="font-size:0.6em; text-align:left; margin-left:10px; font-family:arial"><color="000000"><br/>' + 
				...
				//Block 2
				middleBlock : '<div><p style="font-size:0.6em; text-align:left; margin-left:10px; font-family:arial"><color="000000"><br/>' + 
				...
				//Block 3
				lastBlock : '<div><p style="font-size:0.6em; text-align:left; margin-left:10px; font-family:arial"><color="000000"><br/>' + 
		...
		{
			return ({
				data: {condition: inPrimeCat + "/" + inTargetCat},
				inherit:{set: 'basicTrial'},
				stimuli: [
					{ inherit: {set: inTargetCat, type:'exRandom'}, data : {handle:'targetStim'} },
					{ inherit: {set: inPrimeCat, type:'exRandom'}, data : {handle:'primeStim'} },
		...
		for (var iCat=0; iCat < primeCats.length; iCat++)
		{
			...
			API.addStimulusSets(primeCats[iCat].name, 
			[{
				data : {alias:primeCats[iCat].name},
				inherit:'Default',
				media: {inherit:{type:'exRandom',set:primeCats[iCat].name}}, 
				css : piCurrent.primeStimulusCSS
			}]);
			API.addMediaSets(primeCats[iCat].name, primeCats[iCat].mediaArray);
		}

		API.addStimulusSets(targetCats.rightAttTargets.name, 
		[{
			data : {wordCategory:targetCats.rightAttTargets.name, alias:targetCats.rightAttTargets.name},
			inherit:'Default',
			media: {inherit:{type:'exRandom',set:targetCats.rightAttTargets.name}}, 
			css : targetCats.rightAttTargets.stimulusCSS
		}]);
		
		API.addStimulusSets(targetCats.leftAttTargets.name, 
		[{
			data : {wordCategory:targetCats.leftAttTargets.name, alias:targetCats.leftAttTargets.name},
			inherit:'Default',
			media: {inherit:{type:'exRandom',set:targetCats.leftAttTargets.name}},
			css : targetCats.rightAttTargets.stimulusCSS
		}]);
